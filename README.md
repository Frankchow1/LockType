...
3. **首次打开**：因为没买 Apple 开发者证书（$99/年），系统会拦一下，按下方「首次打开」处理一次即可
4. 看到菜单栏出现锁图标 = 安装成功

### ⚠️ 首次打开（被「无法验证开发者」拦住时）

> 这是未公证 App 的正常现象，**只需处理一次**，之后双击就能直接开。
...
跑完直接双击打开，不会再有任何弹窗。（原理：移除下载文件的「隔离」标记。）

**或者用图形界面**（分系统版本，步骤不同）：

- **macOS 15 Sequoia 及以上**：双击 → 弹出「未能打开」→ 打开 **系统设置 → 隐私与安全性** → 滑到底部，会看到一行「已阻止 LockType…」→ 点 **仍要打开** → 再确认一次
  - ⚠️ 注意：新系统已经**取消了**老的「右键→打开」绕过方式，必须走系统设置
- **macOS 14 Sonoma 及以下**：**右键（或 Control 点击）** `LockType.app` → **打开** → 弹窗里再点 **打开**
...
A：因为没花钱买 Apple 开发者证书，App 未公证。**只需处理一次**，最快的办法是终端跑：
```bash
xattr -dr com.apple.quarantine /Applications/LockType.app
```
不想用终端就走图形界面：macOS 15+ 到 **系统设置 → 隐私与安全性 → 仍要打开**；macOS 14 及以下 **右键 → 打开 → 打开**。详见上文 [首次打开](#-首次打开被无法验证开发者拦住时)。
